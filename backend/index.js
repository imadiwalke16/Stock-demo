...
// cors and bodyparser needed for sequrity concerns and react linking
...
app.post("/newSellOrder", async (req, res) => {
  ...
  if (mode !== "SELL") {
    return res.status(400).send("Invalid mode. Only 'SELL' mode is allowed.");
  }

  try {
    // Fetch all orders for the stock
    ...
    if (availableQty < qty) {
      return res.status(400).send("Insufficient stock quantity available.");
    }
